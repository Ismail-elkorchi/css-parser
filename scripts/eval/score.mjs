...
async function main() {
  ...
  if (agentPoints > 0) {
    ...
    if (agent) {
      ...
      const enabledFeatureCount = requiredFeatures.filter((featureName) => Boolean(agentFeatures?.[featureName]?.ok)).length;
      const featureCoverage = requiredFeatures.length === 0 ? 0 : (enabledFeatureCount / requiredFeatures.length);
