...
async function main() {
  ...
  const lines = [
    ...
    `- fixture memory retained delta MB: ${String(selectorFixtureBenchmark?.memoryRetainedDeltaMB ?? 0)}`,
    ...
    `- realworld memory retained delta MB: ${String(selectorRealworldBenchmark?.memoryRetainedDeltaMB ?? 0)}`,
    ...
    `- selector fixture memory median delta MB: ${String(selectorStabilityFixture?.memoryRetainedDeltaMB?.median ?? 0)}`,
